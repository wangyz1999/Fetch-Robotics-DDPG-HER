...
class DDPGAgent(object):
    def __init__(self, cfg, env_params):
        ...
        self.actor = Actor(cfg, env_params).cuda()
        self.critic = Critic(cfg, env_params).cuda()
    ...
    def get_sample_experience(self):
        ...
        t_state = torch.tensor(state).cuda()
        t_action = torch.tensor(action).cuda()
        t_reward = torch.tensor(reward).cuda()
        t_next_state = torch.tensor(next_state).cuda()
        t_done = torch.tensor(done).cuda()
